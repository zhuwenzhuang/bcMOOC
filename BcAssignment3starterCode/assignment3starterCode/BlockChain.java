import java.util.ArrayDeque;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.TreeMap;

// Block Chain should maintain only limited block nodes to satisfy the functions
// You should not have all the blocks added to the block chain in memory 
// as it would cause a memory overflow.

public class BlockChain {
	public static final int CUT_OFF_AGE = 10;

	public static final int MAX_BLOCK = 100000;// 
	private TransactionPool myTxnPool;
	private int blockNum;
	private int maxHeight;
	private TreeMap<Integer, LinkedList<BlockWrapper>> heightBlockMap;
	private HashMap<ByteArrayWrapper, BlockWrapper> hashBlockMap;// blockhash ->
																	// block

	/**
	 * create an empty block chain with just a genesis block. Assume
	 * {@code genesisBlock} is a valid block
	 */
	public BlockChain(Block genesisBlock) {
		maxHeight = -1;
		blockNum = 0;
		myTxnPool = new TransactionPool();
		heightBlockMap = new TreeMap<>();
		hashBlockMap = new HashMap<>();

		UTXOPool pool = new UTXOPool();
		{
			Transaction tx = genesisBlock.getCoinbase();
			for (int i = 0; i < tx.getOutputs().size(); i++) {
				Transaction.Output output = tx.getOutputs().get(i);
				byte[] hash = tx.getHash();
				pool.addUTXO(new UTXO(hash, i), output);
			}
		}
		++maxHeight;
		genesisBlock.finalize();
		BlockWrapper bw = new BlockWrapper(genesisBlock, maxHeight, pool);
		LinkedList<BlockWrapper> heightBlockList = new LinkedList<>();
		heightBlockList.add(bw);
		heightBlockMap.put(bw.height, heightBlockList);
		hashBlockMap.put(new ByteArrayWrapper(bw.block.getHash()), bw);
		blockNum ++;

	}

	private boolean addBlockWrapper(Block block, int height, UTXOPool pool) {
		block.finalize();
		BlockWrapper bw = new BlockWrapper(block, height, pool);
		if(!heightBlockMap.containsKey(height)){
			LinkedList<BlockWrapper> heightBlockList = new LinkedList<>();
			heightBlockList.add(bw);
			heightBlockMap.put(bw.height, heightBlockList);
		}else{
			heightBlockMap.get(height).add(bw);
		}
		
		hashBlockMap.put(new ByteArrayWrapper(bw.block.getHash()), bw);
		blockNum ++;
		return true;
	}

	/** Get the maximum height block */
	public Block getMaxHeightBlock() {
		return heightBlockMap.lastEntry().getValue().getFirst().block;
	}

	/** Get the UTXOPool for mining a new block on top of max height block */
	public UTXOPool getMaxHeightUTXOPool() {
		return heightBlockMap.lastEntry().getValue().getFirst().utxoPool;
	}

	/** Get the transaction pool to mine a new block */
	public TransactionPool getTransactionPool() {
		return myTxnPool;//
	}

	/**
	 * Add {@code block} to the block chain if it is valid. For validity, all
	 * transactions should be valid and block should be at
	 * {@code height > (maxHeight - CUT_OFF_AGE)}.
	 * 
	 * <p>
	 * For example, you can try creating a new block over the genesis block
	 * (block height 2) if the block chain height is {@code <=
	 * CUT_OFF_AGE + 1}. As soon as {@code height > CUT_OFF_AGE + 1}, you cannot
	 * create a new block at height 2.
	 * 
	 * @return true if block is successfully added
	 */
	public boolean addBlock(Block block) {

		if (block.getPrevBlockHash() == null || !hashBlockMap.containsKey(new ByteArrayWrapper(block.getPrevBlockHash()))) {
			return false;
		}

		BlockWrapper pbw = hashBlockMap.get(new ByteArrayWrapper(block.getPrevBlockHash()));
		int height = pbw.height + 1;
		int lastHeight = maxHeight + CUT_OFF_AGE;
		if (height <= lastHeight) {
			return false;
		}
		TxHandler handler = new TxHandler(pbw.utxoPool);
		int txnSize = block.getTransactions().size();
		if (handler.handleTxs(block.getTransactions().toArray(new Transaction[txnSize])).length != txnSize) {
			return false;
		}
		UTXOPool pool = handler.getUTXOPool();
		{
			double count = 0;
			Transaction tx = block.getCoinbase();
			for (int i = 0; i < tx.getOutputs().size(); i++) {
				Transaction.Output output = tx.getOutputs().get(i);
				count += output.value;
				byte[] hash = tx.getHash();
				pool.addUTXO(new UTXO(hash, i), output);
			}
			if (count > Block.COINBASE) {
				return false;
			}
		}

		if(!addBlockWrapper(block, height, pool)){
			return false;
		}
		
		while (heightBlockMap.firstKey() < lastHeight || blockNum > MAX_BLOCK) {
			for (BlockWrapper bw : heightBlockMap.firstEntry().getValue()) {
				hashBlockMap.remove(bw);
				blockNum --;
			}
			heightBlockMap.remove(heightBlockMap.firstKey());
		}
		return true;
	}

	/** Add a transaction to the transaction pool */
	public void addTransaction(Transaction tx) {
		if (tx != null)// 
			myTxnPool.addTransaction(tx);
	}

	class BlockWrapper {
		public BlockWrapper(Block block, int height, UTXOPool pool) {
			super();
			this.block = block;
			this.height = height;
			this.utxoPool = pool;
		}

		public UTXOPool utxoPool;
		public Block block;
		public int height;
	}
}